-- Function to calculate rolling CMV (food cost percentage)
-- This provides a more accurate view than daily purchase-based calculations
...
BEGIN
    ...
    daily_data AS (
        SELECT 
            DATE(fs.submitted_at) as sale_date,
            SUM(CASE 
                WHEN fs.source_form = 'vendas' 
                THEN (fs.data->>'cash')::DECIMAL + 
                     (fs.data->>'card_credit')::DECIMAL + 
                     (fs.data->>'card_debit')::DECIMAL + 
                     (fs.data->>'pix')::DECIMAL
                ELSE 0 
            END) as sales,
            SUM(CASE 
                WHEN fs.source_form = 'pedido' 
                THEN (fs.data->>'order_value')::DECIMAL
                ELSE 0 
            END) as purchases
        FROM form_submissions fs
        CROSS JOIN date_range dr
        WHERE fs.owner_id = p_owner_id
          AND DATE(fs.submitted_at) BETWEEN dr.start_date AND dr.end_date
          AND fs.source_form IN ('vendas', 'pedido')
...
-- Example usage:
-- SELECT * FROM get_rolling_cmv('owner-uuid', '2024-01-15', 30);
