// supabase/functions/end-message-demo/index.ts
// Sends end_message_demo template to specified owner via WhatsApp
...
serve(async (req) => {
  console.log("[END-DEMO] Function started", new Date().toISOString());
  ...
  try {
    ...
    const TEMPLATE_NAME = "end_message_demo";

    console.log("[END-DEMO] Environment check:", {
    ...
    if (!ownerId && !phoneNumber) {
      ...
      const ENV_TO = Deno.env.get("OWNER_PHONE_E164") || Deno.env.get("END_DEMO_TO") || undefined;
      ownerId = ENV_OWNER || null;
      phoneNumber = ENV_TO || null;
    }

    console.log("[END-DEMO] Request params:", {
      ownerId,
      phoneNumber,
      url: req.url
    });
    ...
    if (phoneNumber) {
      ...
      console.log("[END-DEMO] Using provided phone number:", targetPhone);
    } 
    // Otherwise, fetch from owner record
      ...
      if (error) {
        console.error("[END-DEMO] Error fetching owner:", error);
        return jsonResponse({
          success: false,
      ...
      if (!resolvedPhone) {
        ...
        if (!e2 && (data2 as OwnerPhoneRow)?.phone_e164) {
          ...
          console.error("[END-DEMO] Owner has no phone number");
          return jsonResponse({
            success: false,
      ...
      console.log("[END-DEMO] Found owner:", {
        id: owner.id,
    ...
    console.log("[END-DEMO] Sending WhatsApp request to:", waUrl);
    console.log("[END-DEMO] Request body:", JSON.stringify(waBody, null, 2));
    ...
    console.log("[END-DEMO] WhatsApp response status:", waResponse.status);
    ...
    console.log("[END-DEMO] WhatsApp response body:", JSON.stringify(waResult, null, 2));
    ...
    if (ownerId) {
      ...
      if (logError) {
        console.error("[END-DEMO] Failed to log delivery:", logError);
      } else {
        console.log("[END-DEMO] Delivery logged successfully");
      }
    }

    // Check if WhatsApp request was successful
    if (!waResponse.ok) {
      console.error("[END-DEMO] WhatsApp API error:", waResult);
      return jsonResponse({
        success: false,
    ...
    return jsonResponse({
      success: true,
      message: "End demo message sent successfully",
      result: {
        to: targetPhone,
    ...
    console.error("[END-DEMO] Fatal error:", error);
    return jsonResponse({
      success: false,
